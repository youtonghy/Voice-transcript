    ...
    try:
        ...
        if len(audio_data) > 0:
            ...
            if transcription:
                ...
                # 自动输入转写结果到当前光标位置
                auto_input_transcription(transcription)
...
def auto_input_transcription(text):
    """自动输入转写结果到当前光标位置"""
    ...
    try:
        ...
        # 清理文本，去除首尾空白并确保末尾有适当的结束
        clean_text = text.strip()
        
        # 使用 keyboard 库输入文字
        keyboard.write(clean_text, delay=0.01)  # 每个字符间隔0.01秒
